...
- **Personal sidebar settings** — `/settings` now has Account and Sidebar tabs. Sidebar order, visibility, and custom internal/http(s) links are stored locally, rendered by `SiteSidebarNav`, and resettable without changing permissions or schema (sources: `app/settings/page.tsx`, `app/settings/sidebar-settings.tsx`, `lib/sidebar-preferences.ts`, `app/site-sidebar-nav.tsx`).
