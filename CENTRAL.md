...
- **Cross-surface reliability and privacy audit** — mobile navigation is inert when closed and contains focus while open; Settings accepts only canonical bounded preferences and serializes optimistic sidebar saves; attempt details require an exact perfect score or `admin:analytics` before sensitive responses are queried; hidden leaderboard users remain anonymous in per-set submissions; writeup and announcement mutations reject cross-site requests and use race-safe persistence/cleanup. Shared query helpers now bound repeated search and page parameters across public/admin directories, the Users directory uses the same visible-set performance model as the leaderboard, and patched framework/database/parser dependencies audit with zero known vulnerabilities. Signed-out theme changes remain browser-local instead of issuing guaranteed-unauthorized Settings requests. No schema or deploy-flow change is required (sources: `app/global-mobile-nav.tsx`, `app/api/settings/route.ts`, `app/attempts/[id]/page.tsx`, `lib/submissions.ts`, `app/api/admin/announcements/route.ts`, `app/api/problem-sets/[id]/writeups/route.ts`, `lib/query-params.ts`, `app/users/page.tsx`, `app/theme-toggle.tsx`, `package.json`).
...
- **Admin Panel and batch ZIP controls** — staff navigation now exposes one `/admin` sidebar entry. The permission-aware panel groups content, people/classes, and insights/operations tools, while the JSON batch importer offers explicit dry-run-all, publish/draft-all, and upload-all actions with bounded concurrency and progress (sources: `app/admin/page.tsx`, `app/site-sidebar.tsx`, `app/admin/import/json-zip-import-panel.tsx`, `lib/permissions.ts`).
- **Personal sidebar settings** — `/settings` now has Account and Sidebar tabs. Sidebar order, visibility, and selected permission-filtered Admin Panel tools persist in `User.sidebarPreferences`, with an account-scoped local cache for immediate navigation updates. Arbitrary custom labels and URLs are not accepted. Preferences are rendered by `SiteSidebarNav` and resettable without weakening route authorization (sources: `app/settings/page.tsx`, `app/settings/sidebar-settings.tsx`, `app/api/settings/route.ts`, `lib/sidebar-preferences.ts`, `app/site-sidebar-nav.tsx`).
- **Shared responsive regression pass** — Settings now uses one unframed workspace around conventionally bordered controls, every form control has a programmatic label, and its header actions fit narrow screens. The Students table's mobile card layout is excluded from generic minimum-width table rules. Reserving the document scrollbar gutter prevents the fixed mobile menu button from shifting horizontally when opening the sidebar, and the Anime.js caret animation uses the v4 `steps(...)` easing helper without console deprecation warnings (sources: `app/settings/page.tsx`, `app/typewriter-greeting.tsx`, `app/globals.css`, `tests/browser_harness_smoke.py`).
...
- **Openable writeups and images** — directory cards now deep-link to the exact post on its problem set writeups page through both the title and an explicit `Open writeup` action. The target post scrolls into view and receives a visible highlight. Attached images are keyboard-accessible full-size links that open in a new tab with a visible zoom affordance; vote and delete controls remain independent (sources: `app/problem-sets/[slug]/writeups/writeups-client.tsx`, `lib/writeup-links.ts`).
...
- **Evidence-aware performance analytics** — the old points-weighted best average and duplicated leaderboard score are replaced by one derived `PerformanceProfile` in `lib/analytics.ts`. Mastery Index combines a three-set-prior proficiency score (65%), square-root visible-set breadth (20%), and a smoothed lower-quartile consistency floor (15%); best-set average, mastery rate, counts, and evidence remain visible. Dashboard, profiles, Users directory, student admin/detail, global analytics leaders, leaderboard, and student CSV use the same helper; Settings intentionally no longer loads removed performance cards. `npm run simulate:performance` validates a deterministic 100-student × 100-set cohort: `0.980` ability/index rank correlation, 89 distinct values, monotonic ability deciles, and broad 80% performance outranking one perfect result. No schema/deploy change is required (sources: `lib/analytics.ts`, `app/users/page.tsx`, `app/api/settings/route.ts`, `scripts/simulate-performance-model.ts`, `tests/analytics.test.ts`).
- **Attempt review** — `/attempts/[id]` presents a DBSOJ-inspired submission summary and dense expandable question rows with correct/incorrect/skipped state, marks, topics, and verdicts. Ordinary students can open only their own attempt, but submitted/normalized answers, accepted answers, grader notes, explanations, and full statements/assets stay redacted until the viewer has an exact perfect attempt on that set; `admin:analytics` can inspect them directly. Unauthorized attempt IDs return 404. Review links appear immediately after submit, on solved-set locks and set attempt history, on student dashboard history, and in student/per-set admin analytics. Pure summary/status helpers live in `lib/attempt-review.ts`, and `proxy.ts` includes `/attempts/:path*` (sources: `app/attempts/[id]/page.tsx`, `app/problem-sets/[slug]/answer-grid.tsx`, `app/problem-sets/[slug]/page.tsx`, `app/dashboard/page.tsx`, `app/admin/students/[id]/page.tsx`, `app/admin/sets/[id]/analytics/page.tsx`, `proxy.ts`).
