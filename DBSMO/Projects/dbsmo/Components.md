...
- `LatexStatement` in `app/problem-sets/[slug]/latex-statement.tsx`: server-side KaTeX renderer for LaTeX/HTML-style statements and imported image assets. Its escape-aware tokenizer handles delimited math, multiline math, matching bare display environments, escaped currency, and image tokens; HTML math tags become unambiguous TeX delimiters while other tags remain inert. It loads `mhchem` and applies `lib/latex-compat.ts` for tables (including optional `[t]`/`[c]` position arguments), legacy display environments, contest currency, paired-delimiter/calculus/blackboard aliases, basic SI-unit notation, and safely portable MathLive notation aliases. Rendering preserves `trust: false`, `globalGroup: false`, fresh macros per expression, and explicit expansion/size limits (sources: named files, `tests/latex-statement.test.ts`, `docs/latex-support.md`).
