...
Choice previews accept delimiter-free standalone expressions when a LaTeX command appears inside math-shaped input, including mixed fractions such as `18\\frac{1}{2}` and radicals such as `5\\sqrt{2}-7`. Delimiters remain recommended for prose with embedded math; rendering continues through `LatexStatement` with KaTeX `trust: false` and bounded expansion (sources: `app/problem-sets/[slug]/latex-statement.tsx`, `docs/latex-support.md`, `tests/latex-statement.test.ts`).
