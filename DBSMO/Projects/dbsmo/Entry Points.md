...
- `/settings` - client Account/Sidebar settings: display name, avatar URL/upload, Google profile-picture fallback, privacy flags, theme, typewriter settings, and local sidebar order/visibility/custom links (sources: `app/settings/page.tsx`, `app/settings/sidebar-settings.tsx`, `lib/sidebar-preferences.ts`, `app/api/settings/route.ts`).
