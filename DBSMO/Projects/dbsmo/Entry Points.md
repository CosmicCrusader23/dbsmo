...
- `/settings` - client Account/Sidebar settings: display name, avatar URL/upload, Google profile-picture fallback, privacy flags, theme, typewriter settings, account-scoped sidebar order/visibility, drag ordering, and permission-aware Admin Panel tool selection (sources: `app/settings/page.tsx`, `app/settings/sidebar-settings.tsx`, `lib/sidebar-preferences.ts`, `lib/admin-tools.ts`, `app/api/settings/route.ts`).
