...
- Hardened attempt privacy so partial-score owners cannot inspect answer keys, over-max scores cannot count as perfect, sensitive response rows are fetched only after authorization, and hidden leaderboard identities remain anonymous in the public submission list.
- Hardened Settings persistence with strict/canonical JSON validation, serialized optimistic sidebar saves with rollback, one always-reachable trusted navigation link, stable API error handling, and removal of the obsolete all-attempt statistics query.
- Fixed mobile navigation accessibility by making the closed sheet inert and unfocusable, making background content inert while it is open, restoring focus to the toggle on close, and preserving visible keyboard focus rings.
- Added cross-site mutation checks and race-safe delete/create behavior to writeups and class announcements. Writeup image compensation is now reference-aware, and invalid writeup fields are rejected instead of silently coerced or truncated.
- Standardized repeated and oversized query-parameter handling across catalog, user, audit, analytics, leaderboard, classes, writeups, settings-adjacent, and admin pagination surfaces so malformed URLs cannot trigger array/string crashes.
