...
export default async function UsersPage({ searchParams }: { searchParams?: UsersSearchParams }) {
  ...
  const [users, problemSets] = await Promise.all([
  ...
  const userRows = users
    .map((u) => {
      const performance = computePerformanceProfile(
        u.attempts.filter((attempt) => visibleSetIds.has(attempt.problemSetId)),
        visibleSetIds.size,
      );
      return {
        ...u,
            ...
            <Link key={u.id} href={profilePathFromEmail(u.email)} className="user-card-link">
              <div className="user-card">
                ...
                <div className="user-card-stats">
                  <span>{u.performance.masteryIndex.toFixed(1)} mastery</span>
                  <span>{u.performance.bestSetAverage.toFixed(1)}% best avg</span>
                </div>
              </div>
            </Link>
          ))
